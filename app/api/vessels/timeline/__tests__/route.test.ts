...
describe("GET /api/vessels/timeline", () => {
  ...
  it("returns 200 with timeline data on success", async () => {
    ...
    mockGetTrafficTimeline.mockResolvedValueOnce(
      mockData as unknown as Awaited<ReturnType<typeof getTrafficTimeline>>,
    );
  ...
  it("defaults to 24 hours when no hours param provided", async () => {
    mockGetTrafficTimeline.mockResolvedValueOnce(
      [] as unknown as Awaited<ReturnType<typeof getTrafficTimeline>>,
    );
